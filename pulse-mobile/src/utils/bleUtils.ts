...
export const listenOverBle = (
  ...
  bleManager.startDeviceScan(
    ...
    (error, device) => {
      ...
      deviceCount++;
      if (deviceCount % 10 === 0) {
        console.log(`📡 Scanned ${deviceCount} devices, found ${dataCount} data packets`);
      }
      ...
      if (serviceDataB64) {
        try {
          ...
          dataCount++;
          console.log(`📨 Received service data packet (${chunk.length} bytes) from ${device.name || 'unknown'}`);
        } catch (e) {
        ...
        try {
          ...
          if (
            ...
            dataCount++;
            console.log(`📨 Received manufacturer data packet (${chunk.length} bytes) from ${device.name || 'unknown'}`);
          }
        } catch (e) {
  ...
  return () => {
    ...
    console.log(`🛑 BLE Scan stopped. Scanned ${deviceCount} devices, found ${dataCount} data packets.`);
  };
};
